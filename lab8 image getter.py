...
class MovingObjectsBBSDrawer:
    def __init__(self,filter,obj_name):
        self.filter=filter
        self.obj_name=obj_name
        self.counter=len(os.listdir("images/%s"%obj_name))
    ...
    def try_draw(self,img,K,world_2_camera ,world,vehicle,camera_rgb,record,check_orientation=False):
        ...
        for npc in world.get_actors().filter(self.filter):
            ...
            if npc.id != vehicle.id:
                ...
                if dist < 20 and dist>3 and (not check_orientation or abs_diff_rot<30 or abs_diff_rot>330):
                    ...
                    if forward_vec.dot(ray) > 1 and np.arccos(forward_vec.dot(ray) / np.sqrt(forward_vec.dot(forward_vec) * ray.dot(ray)))<np.pi/4:
                        ...
                        if endpoint[0]-startpoint[0]<300 and endpoint[1]-startpoint[1]<300 and record and cu.save_img_fragment(img,startpoint,endpoint,self.obj_name,self.counter,resolution=(64,128)):
                            self.counter+=1
                        old_startpoint=startpoint
                        old_endpoint=endpoint
                        
                        startpoint=(startpoint[0]-np.random.randint(15)-5,startpoint[1]-np.random.randint(15)-5)
                        endpoint=(endpoint[0]+np.random.randint(15)-5,endpoint[1]+np.random.randint(15)-5)
                        if endpoint[0]-startpoint[0]<300 and endpoint[1]-startpoint[1]<300 and record and cu.save_img_fragment(img,startpoint,endpoint,self.obj_name,self.counter,resolution=(64,128)):
                            self.counter+=1
                        
                        detected=True
                        cv2.rectangle(img,old_startpoint,old_endpoint,(0,255,255,0),1)
        return detected
...
def main():
    ...
    try:
        ...
        pedes_drawer=MovingObjectsBBSDrawer('*walker.pedestrian*',"pedestrian/")
        controller=KeyboardControl(world,vehicle,True)
        counter=len(os.listdir("images/empty_pedestrians"))
        ...
        with CarlaSyncMode(world, camera_rgb,fps=30) as sync_mode:
            while True:
                ...
                detected=pedes_drawer.try_draw(img,K,world_2_camera,world,vehicle,camera_rgb,controller.recording)  
                if controller.recording and not detected:
                    for i in range(50):
                        randomy=np.random.randint(0,img.shape[0]-100)
                        randomx=np.random.randint(0,img.shape[1]-200)
                        xshape=np.random.randint(0,100)
                        if classify_img(img[randomx:randomx+2*xshape,randomy:randomy+xshape,:],hog,norm,clf)==1:
                            if cu.save_img_fragment(img,(randomy,randomx),(randomy+xshape,randomx+2*xshape),"empty_pedestrians/",counter,resolution=(64,128)):
                                counter+=1 
